...
import { haversineMeters, latLngBoundingBox } from "./utils/geo";
...
type IssueStatus = "pending_triage" | "open" | "in_progress" | "resolved" | "merged";

interface NearbyIssueMeta {
  id: string;
...
interface TriageAgentResponse {
  ...
  aiConfidence: number;
  photoAltText: string;
  resolutionSteps: string[]; // FIX Gap 2: Gemini-generated action plan
}
...
const TRIAGE_FALLBACK = {
  ...
  mergeTargetId: null,
  category: "uncategorized" as IssueCategory,
  priorityScore: 50,
...
async function runCloudVision(photoURL: string): Promise<string[]> {
  ...
  try {
    ...
    return (response.labelAnnotations ?? [])
      .filter((a: { score: number }) => a.score > 0.6)
...
async function getProximateIssues(
  lat: number,
  lng: number,
  excludeId: string
): Promise<NearbyIssueMeta[]> {
...
async function runGeminiAgent(
  photoURL: string,
  userDescription: string, // already sanitized before calling
  visionLabels: string[],
  nearbyIssues: NearbyIssueMeta[]
): Promise<TriageAgentResponse> {
  ...
  const nearbyCtx = nearbyIssues.length
    ? nearbyIssues.map(
        n => `- id:${n.id} | category:${n.category} | priority:${n.priorityScore} | distance:${n.distanceMeters}m | verifications:${n.verifyCount}`
      ).join("\n")
    : "None";
...
Return ONLY valid JSON. No markdown. No text outside the JSON object:
{
  ...
  const parsed = JSON.parse(text) as TriageAgentResponse;
...
async function runTriagePipeline(issueId: string, data: FirebaseFirestore.DocumentData) {
...
export const triageIssue = onDocumentCreated(
  {
  ...
  async (event) => {
    ...
    if (!data || data.status !== "pending_triage") return;
...
export const retryTriage = onSchedule(
  {
...
export const computeWardHealth = onDocumentUpdated(
  {
    document: "issues/{issueId}",
  ...
  async (event) => {
    ...
    const wardId: string = after.wardId;
    if (!wardId) return;
...
export const updateIssueStatus = onCall(
  { region: REGION },
  async (request) => {
    if (!request.auth) throw new HttpsError("unauthenticated", "Must be signed in");

    const uid = request.auth.uid;
    const { issueId, status } = request.data as { issueId: string; status: IssueStatus };

    if (!issueId || !status) {
      throw new HttpsError("invalid-argument", "issueId and status are required");
    }
...
export const verifyIssue = onCall(
  { region: REGION },
  async (request) => {
    if (!request.auth) throw new HttpsError("unauthenticated", "Must be signed in");

    const uid = request.auth.uid;
    const { issueId } = request.data as { issueId: string };
    ...
    const result = await db.runTransaction(async (tx) => {
      ...
      if (user?.lastVerifyAt) {
        const lastVerify: FirebaseFirestore.Timestamp = user.lastVerifyAt;
        const secsSince = (Date.now() - lastVerify.toMillis()) / 1000;
        if (secsSince < 60) {
          throw new HttpsError(
            "resource-exhausted",
            `Please wait ${Math.ceil(60 - secsSince)} seconds before verifying again.`
          );
        }
      }

      // Prevent double-verify on the same issue
      if ((issue.verifiedBy as string[] || []).includes(uid)) {
        throw new HttpsError("already-exists", "You have already verified this issue");
      }

      const newVerifyCount = (issue.verifyCount as number || 0) + 1;
...
export const flagIssue = onCall(
  { region: REGION },
  async (request) => {
    if (!request.auth) throw new HttpsError("unauthenticated", "Must be signed in");

    const uid = request.auth.uid;
    const { issueId, reason } = request.data as { issueId: string; reason?: string };
    if (!issueId) throw new HttpsError("invalid-argument", "issueId is required");
    ...
    await db.runTransaction(async (tx) => {
      ...
      if (flaggedBy.includes(uid)) {
        throw new HttpsError("already-exists", "You have already flagged this issue");
      }
...
export const escalateStaleIssues = onSchedule(
  {
...
export const generatePredictions = onSchedule(
  {
    schedule: "every 24 hours",
    secrets: ["GEMINI_API_KEY"],
    timeoutSeconds: 540,
...
Return ONLY valid JSON, no markdown:
{
  ...
  "severity": "low" | "medium" | "high" | "critical"
        ...
        if (locationIssues.length > 0) {
          lat = locationIssues.reduce((s: number, i: any) => s + i.location.latitude, 0) / locationIssues.length;
          lng = locationIssues.reduce((s: number, i: any) => s + i.location.longitude, 0) / locationIssues.length;
        }
...
export const escalateStaleIssuesV2 = onSchedule(
  {
  ...
  async () => {
    ...
    for (const issueDoc of staleSnap.docs) {
      ...
      const currentPriority: number = data.priorityScore || 50;
...
export const awardBadges = onDocumentUpdated(
  {
  ...
  async (event) => {
    ...
    const currentBadges: string[] = after.badges || [];
    const newBadges: string[] = [];

    const xp: number = after.xpPoints || 0;
    const level = Math.floor(xp / 150) + 1;
    const reportCount: number = after.reportsCount || 0;
    const verifyCount: number = after.verifyCount || 0;
...
export const autoAssignMission = onDocumentUpdated(
  {
    document: "issues/{issueId}",
    region: REGION,
    timeoutSeconds: 60,
  ...
  async (event) => {
    ...
    if (before.status !== "pending_triage" || after.status !== "open") return;
    if ((after.priorityScore || 0) < 80) return;
...
export const verifyDispute = onCall(
  {
    secrets: ["GEMINI_API_KEY"],
    timeoutSeconds: 60,
    memory: "512MiB",
  ...
  async (request) => {
    ...
    if (!issueId || !disputePhotoBase64) {
      throw new HttpsError("invalid-argument", "Missing issueId or dispute photo");
    }

    const docRef = db.collection("issues").doc(issueId);
    const docSnap = await docRef.get();
    if (!docSnap.exists) throw new HttpsError("not-found", "Issue not found");
    const issue = docSnap.data() as any;

    if (issue.status !== "pending_verification") {
      throw new HttpsError("failed-precondition", "Issue is not pending verification");
    }
...
Return ONLY valid JSON:
{
  ...
  "reasoning": "Detailed visual analysis (e.g., 'Damage pattern still visible... Original pothole depth visible in dispute photo... Resolution claim rejected')"
    ...
    if (originalPart) { parts.push({ text: "Original Damage:" }); parts.push(originalPart); }
    if (resolutionPart) { parts.push({ text: "Claimed Resolution:" }); parts.push(resolutionPart); }
...
export const checkJurisdiction = onCall(
  {
    secrets: ["GEMINI_API_KEY"],
    timeoutSeconds: 60,
    memory: "256MiB",
  ...
  async (request) => {
    ...
    if (!issue.location || !issue.department) {
       throw new HttpsError("failed-precondition", "Issue missing location or department");
    }
...
Return JSON: { "correct": boolean, "actual_department": "string", "reasoning": "string" }`;
    ...
    try {
      ...
      throw new HttpsError("internal", "AI failed to analyze jurisdiction");
    }
...
export const verifyResolution = onDocumentUpdated(
  {
    document: "issues/{issueId}",
    secrets: ["GEMINI_API_KEY"],
    timeoutSeconds: 120,
